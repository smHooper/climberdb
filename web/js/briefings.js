class ClimberDBBriefings extends ClimberDB {
	...
	*/
	...
	/*
	Export the current daily briefing schedule as a PDF. This doesn't currently work because 
	WeasyPrint doesn't support grid layout (yet?)
	*/
	briefingScheduleToPDF() {
		var css = '',
			deferreds = [],
			headInnerHTML = '';
		for (const el of $('head > link')) {
			const href = $(el).attr('href');
			if (href.startsWith('css/')) {
				deferreds.push($.get({url: href}).done(cssString => {css += cssString + '\n'}))
			} else {
				headInnerHTML += el.outerHTML + '\n' 
			}
		}

		const sidebarHTML = $('.briefing-details-sidebar').html();
		...
		$.when(...deferreds).then(() => {
			...
			<body>
				<div class="briefing-details-sidebar">
				${sidebarHTML}
				</div>
			</body>
			`
			$.post({
				...
				data: {
					html: html,
					date: $('.calendar-cell.selected').data('date')
				}
			}).done(responseData => {
				if (this.pythonReturnedError(responseData)) {
					showModal('The briefing schedule could not be exported because of an unexpected error: ' + responseData, 'Unexpected Error');
					return;
				}
	...
	/*
	Download the briefing schedule as a PNG. This works, but it's slow and
	the background of the briefing schedule doesn't render correctly so it
	looks too dark
	*/
	briefingScheduleToPNG() {
		const screenshotTarget = document.querySelector('.briefing-details-sidebar');
		const currentDate = $('.calendar-cell.selected').data('date');
		html2canvas(screenshotTarget).then((canvas) => {
			const link = document.createElement('a');
			link.download = `briefing_schedule_${currentDate}.png`;
